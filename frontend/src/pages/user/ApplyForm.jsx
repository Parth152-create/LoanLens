...
export default function ApplyForm() {
  ...
  const [result, setResult]       = useState(null);   // { probability, shapValues }
  ...
  const onStep2Submit = async (data) => {
    ...
    try {
      ...
      // Run ML prediction + save to DB in parallel
      const [prediction, saved] = await Promise.all([
        predict(features),
        submitLoanApplication(loanReq).catch(() => null), // non-blocking
      ]);

      setResult({ ...prediction, formValues: features });
                ...
                {shapTags && (
                  <div style={{ marginTop: "1.75rem" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                      {shapTags.hurts.map((t) => (
                        <FactorTag key={t.feature} type="hurts" label={t.label} />
                      ))}
                      {shapTags.caution.map((t) => (
                        <FactorTag key={t.feature} type="caution" label={t.label} />
                      ))}
                      {shapTags.helps.map((t) => (
                        <FactorTag key={t.feature} type="helps" label={t.label} />
                      ))}
                    </div>
                  </div>
                )}
